...
$sql = "SELECT students.id, students.name, students.enrollment_date, students.end_date, classes.name as class_name, classes.num_group, students.paid FROM students JOIN classes ON students.class_id = classes.id";
...
<body>
    ...
    <div class="main-content">
        ...
        <a href="#" class="btn btn-primary mb-3" data-bs-toggle="modal" data-bs-target="#pdfOptionsModal">Download PDF</a>
                            ...
                            <div class="mb-3" id="classSelect">
                                ...
                                <select id="class_id" name="class_id" class="form-select">
                                    <?php while ($row_class = $result_classes->fetch_assoc()): ?>
                                        <option value="<?php echo $row_class['id']; ?>"><?php echo $row_class['name']; ?></option>
                                    <?php endwhile; ?>
                                </select>
                            </div>
        ...
        <a href="#" class="btn btn-primary mb-3" data-bs-toggle="modal" data-bs-target="#addStudentModal">Add New Student</a>
                ...
                <?php if ($result->num_rows > 0): ?>
                    <?php while($row = $result->fetch_assoc()): ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="#" class="btn btn-warning edit-student" data-id="<?php echo $row['id']; ?>" data-bs-toggle="modal" data-bs-target="#editStudentModal">Edit</a>
        ...
        $(document).ready(function() {
            ...
            $('#sort').on('change', function() {
                ...
                rows.sort(function(a, b) {
                    var keyA = $(a).children('td').eq($('#sort option').index($('#sort option[value="' + sortBy + '"]'))).text().toLowerCase();
                    var keyB = $(b).children('td').eq($('#sort option').index($('#sort option[value="' + sortBy + '"]'))).text().toLowerCase();
